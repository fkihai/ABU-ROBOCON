...
void Run() {
  ...
  if (CMD == "START") {
    SET_RPM = sRPM.toFloat();
    KP      = sKP.toFloat();
    KI      = sKI.toFloat();
    KD      = sKD.toFloat();

    //    RPMbelakang( SET_RPM, KP, KI, KD);
    //    RPMkanan   ( SET_RPM, KP, KI, KD);
    RPMkiri    ( SET_RPM, KP, KI, KD);
  }
  else if (CMD == "STOP") {
    SET_RPM = 0;
    KP      = 0;
