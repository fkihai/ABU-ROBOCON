  ...
  while (Zone1) {
    ...
    while (Auto3) {
      ...
      if (Slave.Data == 'N') {
        ...
        RPSDepan(-(vrps1), 0.05, 0.05);
        RPSBelakang(-(vrps2), 0.05, 0.05);
        RPSKanan(-(vrps3), 0.05, 0.05);
        RPSKiri(-(vrps4), 0.05, 0.05);
        if (digitalRead(Prox) == LOW) {
          ...
          RPSDepan(-(vrps1), 0.01, 0.05);
          RPSBelakang(-(vrps2), 0.01, 0.05);
          RPSKanan(-(vrps3), 0.06, 0.05);
          RPSKiri(-(vrps4), 0.06, 0.05);
          if (digitalRead(LimitSwitch) == LOW) {
            analogWrite(KA_Lpwm_PIN, 0);
            analogWrite(KA_Rpwm_PIN, LOW);
            analogWrite(KI_Lpwm_PIN, 0);
    ...
    while (Auto4) {
      ...
      if (Slave.Data == 'L') {
        ...
        if (Pos.Y < 34) {
          ...
          RPSDepan(-(vrps3), 0.01, 0.05);
          RPSBelakang(-(vrps4), 0.01, 0.05);
          SoftWheel(30, 60, 10, 10, Pos.Y, 5);  // Vrps 1 & 2
          SoftWheel1(30, 60, 10, 10, Pos.Y, 3); // Vrps 3 & 4
        } else if (Pos.Y > 38) {
          SlaveSerial.print('Q');
          Stop();
          delay(20);
          ...
          SoftWheel1(20, 60, 10, 10, Pos.X, 0); // Vrps 3 & 4
          if (Pos.X > 31) {
...
void SoftRigh(float pos) {
  ...
  while (Geser) {
    if (Move) {
      RPSDepan(vrps1, 0.01, 0.05);
      RPSBelakang(vrps2, 0.01, 0.05);
      SoftWheel(pos - 20 , pos, 10, 10, Pos.X, 2);
      analogWrite(KA_Rpwm_PIN, LOW);
      analogWrite(KA_Lpwm_PIN, LOW);
      analogWrite(KI_Rpwm_PIN, LOW);
      analogWrite(KI_Lpwm_PIN, LOW);
...
void SoftLeft(float pos) {
  ...
  while (Geser) {
    if (Move) {
      RPSDepan(-(vrps1), 0.01, 0.05);
      RPSBelakang(-(vrps2), 0.01, 0.05);
      SoftWheel(pos, pos + 15, 10, 10, Pos.X, 2);
      analogWrite(KA_Rpwm_PIN, LOW);
      analogWrite(KA_Lpwm_PIN, LOW);
      analogWrite(KI_Rpwm_PIN, LOW);
      analogWrite(KI_Lpwm_PIN, LOW);
