  ...
  while (Zone1) {
    ...
    while (Auto1) {
      ...
      while (Gorack) {
        ...
        if (abs(Pos.X) > 180 || digitalRead(Prox) == LOW) {
          delay(100);
          Stop();
          vrps1 = 7;
          vrps2 = 7;
    ...
    while (Auto4) {
      ...
      while (Go && !GoLmpr)
      {
        if (!GoLmpr && Pos.Y < 50)
        {
          ...
          RPSKanan(vrps1, 0.01, 0.05);
          RPSKiri(vrps2, 0.01, 0.05);
          SoftWheel(20, 60, 10, 10, Pos.Y, 2);  // Vrps 1 & 2
          SoftWheel1(20, 60, 10, 10, Pos.Y, 1); // Vrps 3 & 4
        }
        while (Pos.Y > 47 && !GoLmpr)
        {
    ...
    while (Auto5) {
      ...
      if (Joystik.Data == 'P')
      {
        ...
        RPSDepan(-(vrps3 + 3), 0.1, 0.05);
        RPSBelakang(-(vrps4), 0.1, 0.05);
        RPSKanan(-(vrps1), 0.1, 0.05);
        RPSKiri(-(vrps2), 0.1, 0.05);
        SoftWheel(20, 60, 10, 10, abs(Pos.Y), 2);  // Vrps 1 & 2
        SoftWheel1(20, 60, 10, 10, abs(Pos.Y), 1); // Vrps 3 & 4

        if (Berhenti && abs(Pos.Y) >= 40) {
          Stop();
          Reset();
          vrps1 = 8;
          vrps2 = 8;
    ...
    while (Auto6) {
      ...
      if (Slave.Data == 'L') {
        ...
        if (Pos.Y < 34) {
          ...
          RPSKanan(vrps3, 0.01, 0.05);
          RPSKiri(vrps4, 0.01, 0.05);
          SoftWheel(20, 60, 10, 10, Pos.X, 4); // Vrps 1 & 2
          SoftWheel1(20, 60, 10, 10, Pos.X, 6); // Vrps 3 & 4
          if (Pos.X > 52 && !GoLmpr ) {
            SlaveSerial.print('G');
            MoveStop();
            GoPos = false;
...
void MoveLeft(float pos, int RPS1, int RPS2, float param) {
  ...
  while (Geser) {
    if (Move) {
      ...
      SoftWheel(param , pos, 10, 10, abs(Pos.X), 3); // Vrps1 && vrps2
      SoftWheel1(param, pos, 10, 10, abs(Pos.X), 2); // vrps3 && vrps4
