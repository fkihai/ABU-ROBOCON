    ...
    while (Auto1) {
      ...
      while (Gorack) {
        ...
        while (AmbilPanah) {
          RPSDepan(-(vrps1), 0.01, 0.05);
          RPSBelakang(-(vrps2), 0.01, 0.05);
          RPSKanan(-(vrps3), 0.04, 0.05);
          RPSKiri(-(vrps4), 0.04, 0.05);
    ...
    while (Auto4) {
      ...
      if (Slave.Data == 'L')
      {
        ...
        if (abs(Pos.Y) < 6) {
          ...
          RPSKanan(vrps3, 0.01, 0.05);
          RPSKiri(vrps4, 0.01, 0.05);
          SoftWheel(120, 290, 10, 10, Pos.X, 3);  // Vrps 1 & 2
          SoftWheel1(120, 290, 10, 10, Pos.X, 1); // Vrps 3 & 4
          if (Pos.X > 290) {
            analogWrite(KA_Rpwm_PIN, 70);
            analogWrite(KA_Lpwm_PIN, LOW);
            analogWrite(KI_Rpwm_PIN, 70);
            analogWrite(KI_Lpwm_PIN, LOW);
            delay(300);
            Stop();
            delay(100);
