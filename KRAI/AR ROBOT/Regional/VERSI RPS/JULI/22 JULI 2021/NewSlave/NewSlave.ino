/*
  NB : Tong Sebelah Kiri(Dekat)
  a,45,56,0.38,0.05
  NB : Tong Sebelah Kiri(JAUH)
  a,50,58,0.39,0.05 (Capit 3)
  a,50,60,0.39,0.05 (Capit 4 da 5)

  NB : Tong Sebelah Kanan(Dekat)
  a,45,50,0.38,0.05
  NB : Tong Sebelah Kanan(JAUH)
  a,50,55,0.38,0.01
...
#define PW_RPWM_PIN 2
#define PW_LPWM_PIN 3
...
void setup()
{
  ...
  ResetEnc();
  delay(100);
  xTaskCreate(LogData, NULL, configMINIMAL_STACK_SIZE, NULL, 1, NULL);
...
void LogData(void *pvParameters)
{
  for (;;) {
...
void ThrowZone1()
{
  switch (Master.Data) {
    ...
    case 'A':
      ...
      LmprCptType1B(45, 56, 0.38, 0.05);
      CapitStop();
      Master.Data = 'B';
      break;
    case 'B':
      LmprCptType2B(45, 56, 0.38, 0.05);
      CapitStop();
      Serial3.print('B');
      Master.Data = 'Z';
      break;
    case 'C':
      LmprCptType3B(50, 58, 0.39, 0.05);
      CapitStop();
      Serial3.print('C');
      Master.Data = 'Z';
      break;
    case 'D':
      LmprCptType4B(50, 60, 0.39, 0.05);
      CapitStop();
      Master.Data = 'E';
      break;
    case 'E':
      LmprCptType5B(50, 60, 0.39, 0.05);
      CapitStop();
      Master.Data = 'Z';
      break;
...
void LmprCptType1B(float SudutLntr, int RPS, float KP, float KD)
{
  ...
  while (Move)
  {
    while (Geser)
    {
      ...
      if (EncSliderPutar.Angle < -15)
      {
        analogWrite(SP_RPWM_PIN, 60);
        digitalWrite(SP_LPWM_PIN, LOW);
        delay(300);
        analogWrite(SP_RPWM_PIN, 27);
        digitalWrite(SP_LPWM_PIN, LOW);
        Transfer(Capit1A, Capit1B, 200);
        delay(100);
        ResetEnc();
        delay(100);
        pwm = 70;
    ...
    while (Throw)
    {
      ...
      RPSpelontar(RPS, 0.27, 0.01);
      if (EncPelontar.Angle > SudutLntr - 1.0 && EncPelontar.Angle < SudutLntr + 1.0)
      {
    ...
    while (Down)
    {
      ...
      if (EncSliderPutar.Angle < -14)
      {
        analogWrite(SP_RPWM_PIN, 70);
        digitalWrite(SP_LPWM_PIN, LOW);
        delay(300);
        digitalWrite(Brake, LOW);
...
void LmprCptType2B(float SudutLntr, int RPS, float KP, float KD)
{
  ...
  while (Move)
  {
    ...
    while (Persiapan)
    {
      ...
      analogWrite(SP_LPWM_PIN, 200);
      digitalWrite(SP_RPWM_PIN, LOW);
      while (EncSliderPutar.Angle > 15)
      {
        analogWrite(SP_LPWM_PIN, 50);
        digitalWrite(SP_RPWM_PIN, LOW);
        delay(200);
...
void LmprCptType3B(float SudutLntr, int RPS, float KP, float KD)
{
  ...
  while (Move)
  {
    while (Geser)
    {
      ...
      analogWrite(SG_RPWM_PIN, pwm);
      digitalWrite(SG_LPWM_PIN, LOW);
      SoftSpeed(28, 20, Distance, 30);
      if (digitalRead(SwtchGeser) == LOW && Distance > 5)
      {
        Sensor = true;
      }
      while (Sensor)
...
void LmprCptType5B(float SudutLntr, int RPS, float KP, float KD)
{
  ...
  while (Move)
  {
    while (Geser)
    {
      ...
      digitalWrite(SG_LPWM_PIN, LOW);
      while (EncSliderPutar.Angle > 39.0)
      {
...
void Transfer(int capitA, int capitB, int Delay)
{
  ...
  digitalWrite(PW_RPWM_PIN, LOW);
  analogWrite(PW_LPWM_PIN, 95);
  delay(Delay);
  digitalWrite(SliderB, LOW);
  digitalWrite(PW_RPWM_PIN, LOW);
  analogWrite(PW_LPWM_PIN, LOW);
  delay(200);
  digitalWrite(capitB, HIGH);
  delay(100);
  digitalWrite(capitB, LOW);
}
...
void TuningLempar() {
  ...
  /* Versi Dua */
  //  if (Data[0] == "b" && Angle > 0.25 ) {
  //    digitalWrite(Brake, HIGH);
  //    digitalWrite(PL_RPWM_PIN, LOW);
  //    analogWrite(PL_LPWM_PIN, round(18));
  //  } else if (Data[0] == "b" && Angle < -0.25 ) {
  //    digitalWrite(Brake, HIGH);
  //    digitalWrite(PL_LPWM_PIN, LOW);
  //    analogWrite(PL_RPWM_PIN, round(18));
  //  } else if (Data[0] == "b" && Angle < 0.25 && Angle > -0.25 ) {
  //    digitalWrite(Brake, LOW);
  //    RPSpelontar(0, 0.1, 0.05);
  //    ResetEnc();
  //    EncPelontar.pulseCnt = 0.00;
  //    digitalWrite(PL_LPWM_PIN, LOW);
  //    analogWrite(PL_RPWM_PIN, 0);
  //  }
}
...
void trialLempar()
{
  ...
  if (Uncal)
  {
    ...
    while (Throw)
    {
      ...
      digitalWrite(SP_RPWM_PIN, LOW);
      digitalWrite(Brake, HIGH);
      RPSpelontar(50, 0.31, 0.01);
      if (EncPelontar.Angle > 55.0 && EncPelontar.Angle < 57.0)
      {
        digitalWrite(Brake, LOW);
        PL.Pwm = 0;
        digitalWrite(PL_LPWM_PIN, LOW);
    ...
    {
      PlCompute();
      if (Down && EncPelontar.Angle > 0.25)
      {
        digitalWrite(Brake, HIGH);
        digitalWrite(PL_RPWM_PIN, LOW);
        analogWrite(PL_LPWM_PIN, 30);
      }
      else if (Down && EncPelontar.Angle < -0.25)
      {
        digitalWrite(Brake, HIGH);
        digitalWrite(PL_LPWM_PIN, LOW);
        analogWrite(PL_RPWM_PIN, 30);
      }

      else if (Down && EncPelontar.Angle < 0.25 && EncPelontar.Angle > -0.25 )
      {
        digitalWrite(Brake, LOW);
        digitalWrite(PL_RPWM_PIN, LOW);
        analogWrite(PL_LPWM_PIN, LOW);
