...
void ZoneLempar2()
{
  ...
  while (Zone1) {
    ...
    while (Auto1) {
      ...
      {
        RPSKanan(-13, 0.01, 0.05);
        RPSKiri(-13, 0.01, 0.05);
      } else {
        SlaveSerial.print('L');
      ...
      while (Gorack) {
        if (abs(Pos.X) < 600 && digitalRead(Prox) == HIGH)
        {
          ...
          delay(200);
          analogWrite(KA_Rpwm_PIN, LOW);
          analogWrite(KA_Lpwm_PIN, 8);
          analogWrite(KI_Rpwm_PIN, LOW);
          analogWrite(KI_Lpwm_PIN, 8);
          analogWrite(DE_Rpwm_PIN, LOW);
          analogWrite(DE_Lpwm_PIN, 8);
          analogWrite(BE_Rpwm_PIN, LOW);
          analogWrite(BE_Lpwm_PIN, 8);
        }
...
void ThrowZone()
{
  ...
  switch (param)
  {
    ...
    case 'A':
      analogWrite(KA_Rpwm_PIN, 40);
      analogWrite(KA_Lpwm_PIN, LOW);
      analogWrite(KI_Rpwm_PIN, 40);
      analogWrite(KI_Lpwm_PIN, LOW);
