...
void TuningLempar() {
  ...
  if (Joystik.Data == 'P') {
    ...
    analogWrite(KA_Rpwm_PIN, 15);
    analogWrite(KA_Lpwm_PIN, LOW);
    analogWrite(KI_Rpwm_PIN, 24);
    analogWrite(KI_Lpwm_PIN, LOW);
  } else if (Joystik.Data == 'V') {
    Stop();
  }
}
...
void CtrlAuto()
{
  if (Joystik.Data == 'P') Ctrl  = true;
  else if (Joystik.Data == 'V') Ctrl1 = true;
  else if (Joystik.Data == 'X') Ctrl2 = true;
  ...
  while (Zone1) {
    ...
    while (Auto1) {
      ...
      while (GoPos) {
        ...
        SoftWheel(50, 175, 10, 10, abs(Pos.X), 4);  // Vrps 1 & 2
        SoftWheel1(50, 175, 10, 10, abs(Pos.X), 4); // Vrps 3 & 4
    ...
    while (Auto5) {
      ...
      if (Joystik.Data == 'P')
      {
        ...
        if (digitalRead(ProxSmpng) == LOW) {
          ...
          RPSKanan(-(vrps3), 0.09, 0.05);
          RPSKiri(-(vrps4), 0.09, 0.05);
          SoftWheel(30, 100, 10, 10, abs(Pos.Y), 10); // Vrps 1 dan 2
          SoftWheel1(30, 100, 10, 10, abs(Pos.Y), 5); // Vrps 3 dan 4
          if (digitalRead(LimitSwitch) == LOW) {
    ...
    while (Auto6) {
      ...
      if (Slave.Data == 'L') {
        ...
        if (Pos.Y < 34) {
          ...
          if (digitalRead(ProxDpn) == HIGH && Pos.X > 136 ) {
            delay(30);
            analogWrite(DE_Lpwm_PIN, LOW);
            analogWrite(DE_Rpwm_PIN, LOW);
...
void RetryPanah1() {
  ...
  while (Zone1) {
    ...
    while (Auto1) {
      if (abs(Pos.Y) < 60)
      {
        RPSKanan(-30, 0.01, 0.05);
        RPSKiri(-33, 0.01, 0.05);
      } else {
        SlaveSerial.print('L');
        Stop();
        delay(20);
