...
void Bismillah()
{
  ...
  switch (Master.Data) {
    ...
    case 'A':
      LmprCptType1A(50, 96, 0.78, 0.02); // sudut, speed, P, D
      Master.Data = 'a';
      break;
    case 'a':
      Serial3.print('B');
    ...
    case 'b':
      ...
      break;
    case 'C':
      LmprCptType3A(50, 98, 0.78, 0.02); // sudut, speed, P, D
      Master.Data = 'c';
      break;
    case 'c':
      Serial3.print('D');
    ...
    case 'D':
      LmprCptType4A(54, 105, 0.78, 0.02);
      Master.Data = 'd';
      break;
    case 'd':
      Serial3.print('E');
    ...
    case 'E':
      LmprCptType5A(51, 107, 0.78, 0.02); //sudut, speed, P, D
      Master.Data = 'e';
      break;
    case 'e':
      Serial3.print('N');
    ...
    case 'F':
      LmprCptType5B(50, 105, 0.78, 0.02);  //sudut, speed, P, D
      Master.Data = 'f';
      break;
    case 'f':
      Serial3.print('M');
    ...
    case 'G':
      LmprCptType5C(40, 50, 0.33, 0.02); //sudut, speed, P, D
      Master.Data = 'H';
      break;

    case 'H':
      LmprCptType4C(41 , 54, 0.33, 0.02);
      Master.Data = 'h';
    case 'h':
      Serial3.print('I');
      Master.Data = 'Z';
    ...
    case 'I':
      LmprCptType3C(41, 54, 0.33, 0.02); //sudut, speed, P, D
      Master.Data = 'i';
    case 'i':
      Serial3.print('J');
      Master.Data = 'Z';
...
/******* Type A **************/
void LmprCptType1A(float SudutLntr, int RPS, float KP, float KD)
{
  Geser = true;
  Move = true;
  PelontarOn();
  ...
  while (Move)
  {
    ...
    {
      PlCompute();
      if (EncPelontar.Angle < 5.0) {
        RPSpelontar(10, 0.1, 0.05);
      }
      else if (EncPelontar.Angle > 5.0 && EncPelontar.Angle < SudutLntr) {
...
void LmprCptType4A(float SudutLntr, int RPS, float KP, float KD)
{
  ...
  while (Move)
  {
    ...
    {
      PlCompute();
      if (EncPelontar.Angle < 3.0) {
        RPSpelontar(20, 0.1, 0.05);
      }
      else if (EncPelontar.Angle > 3.0 && EncPelontar.Angle < SudutLntr) {
        RPSpelontar(RPS, KP, KD);
      }
      else {
        digitalWrite(Brake, LOW);
...
void LmprCptType4C(float SudutLntr, int RPS, float KP, float KD) {
  ...
  while (Move)
  {
    while (Geser)
    {
      ...
      digitalWrite(SG_LPWM_PIN, LOW);
      digitalWrite(doorlock1, HIGH);
      if (abs(EncSliderPutar.Angle) >= 35) {
        PelontarOff();
        digitalWrite(SP_LPWM_PIN, LOW);
...
void LmprCptType3C(float SudutLntr, int RPS, float KP, float KD) {
  ...
  while (Move)
  {
    while (Putar) {
      ...
      digitalWrite(SP_LPWM_PIN, LOW);
      if (abs(EncSliderPutar.Angle) >= 33) {
        PelontarOff();
        digitalWrite(doorlock1, LOW);
        analogWrite(SP_LPWM_PIN, 60);
        digitalWrite(SP_RPWM_PIN, LOW);
...
void trialThrowing()
{
  ...
  {
    LmprCptType5C(48.0, 64, 0.38, 0.05);
    Ctrlthrow = 'A';
    Coba = false;
  }
  switch (Ctrlthrow)
  {
    case 'A':
      LmprCptType4C(48.0, 64, 0.38, 0.05);
      Ctrlthrow = 'B';
      break;
    case 'B':
